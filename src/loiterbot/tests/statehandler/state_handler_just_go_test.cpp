...
TEST(StateHandlerJustGoTest, StraightAhead)
{
  ...
  LaserAnalysis laser_analysis = LaserAnalysis(true, false, false, 0.0, 4.0l);
  ...
  EXPECT_TRUE(action.get_velocity().value().get_forward() > 2.0l);
...
TEST(StateHandlerJustGoTest, Temp)
{
  LaserCharacteristics laser_characteristics = LaserCharacteristics(LASER_ANGLE_MINIMUM, LASER_ANGLE_INCREMENT, 4ul, 2ul);
  LaserAnalysis laser_analysis = LaserAnalysis(true, false, false, 0.004370, 0.200001l);
  StateHandlerJustGo state_handler = StateHandlerJustGo();
  History history = History();
  history.set_obstacle_last_seen_time(1000.0l, true);
  history.set_time_lost(0.1l);
